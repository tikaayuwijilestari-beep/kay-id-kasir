...
async function generatePDF(
  ...
  // === PAGE 1: Summary + Charts (clean stacked layout) ===
  let page = pdfDoc.addPage([W, H]);
  let y = H - M;

  // Header
  page.drawRectangle({ x: 0, y: y - 40, width: W, height: 52, color: primary });
  page.drawText('KAY ID TOKO BANGUNAN', { x: M, y: y - 8, size: 16, font: fontBold, color: white });
  page.drawText(`${title}  •  ${periodLabel}`, { x: M, y: y - 24, size: 9, font: font, color: rgb(0.85, 0.85, 1) });
  page.drawText(formatCurrency(totalRevenue), { x: W - M - 160, y: y - 12, size: 17, font: fontBold, color: white });
  page.drawText('TOTAL PENDAPATAN', { x: W - M - 140, y: y - 26, size: 7, font: font, color: rgb(0.75, 0.75, 0.95) });
  y -= 52;
  ...
  cards.forEach((c, i) => {
    ...
    page.drawText(c.label, { x: cx + 8, y: y - 12, size: 7, font: font, color: gray500 });
    page.drawText(c.value, { x: cx + 8, y: y - 26, size: 10, font: fontBold, color: c.accent });
    page.drawText(c.sub, { x: cx + 8, y: y - 37, size: 6, font: font, color: gray500 });
  });
  y -= cardH + 16;
  ...
  if (dailyBreakdown.length > 0) {
    ...
    page.drawRectangle({ x: M, y: y - 20, width: contentW, height: 22, color: gray100 });
    page.drawText('PENDAPATAN HARIAN', { x: M + 8, y: y - 14, size: 9, font: fontBold, color: gray700 });
    y -= 28; // Move below header (with gap)

    const maxRev = Math.max(...dailyBreakdown.map((d: any) => d.revenue));
    const barMaxW = contentW - 150;
    const barH = 18;
    ...
    dailyBreakdown.forEach((d: any) => {
      ...
      // Date label on left
      page.drawText(dateShort, { x: M + 6, y: y + 4, size: 8, font: font, color: gray700 });

      // Bar
      const barW = pct * barMaxW;
      page.drawRectangle({ x: M + 65, y: y, width: barW, height: barH, color: primary });
      page.drawRectangle({ x: M + 65 + barW, y: y, width: barMaxW - barW, height: barH, color: gray100 });

      // Value on right
      page.drawText(formatCurrency(d.revenue), { x: M + 65 + barMaxW + 8, y: y + 5, size: 8, font: fontBold, color: gray700 });
      y -= barH + 4;
    });

    // Total line
    y -= 6;
    page.drawLine({ start: { x: M + 6, y: y + 10 }, end: { x: W - M - 6, y: y + 10 }, thickness: 0.5, color: gray200 });
    page.drawText('Total:', { x: M + 6, y: y, size: 8, font: fontBold, color: gray700 });
    page.drawText(formatCurrency(totalRevenue), { x: W - M - 80, y: y, size: 8, font: fontBold, color: success });
    y -= 18;
  }

  // CHART 2: Top Products
  if (topProducts.length > 0) {
    if (y < 100) { page = pdfDoc.addPage([W, H]); y = H - M; }
    // Section header
    page.drawRectangle({ x: M, y: y - 20, width: contentW, height: 22, color: gray100 });
    page.drawText('PRODUK TERLARIS', { x: M + 8, y: y - 14, size: 9, font: fontBold, color: gray700 });
    y -= 28;

    const maxQty = Math.max(...topProducts.map((p: any) => p.qty_sold));
    const barMaxW = contentW - 240;
    const barH = 18;
    ...
    topProducts.slice(0, 10).forEach((p: any, i: number) => {
      ...
      // Rank + name on left
      page.drawText(medal, { x: M + 6, y: y + 4, size: 8, font: fontBold, color: gray700 });
      const name = p.nama_barang.length > 32 ? p.nama_barang.substring(0, 32) + '..' : p.nama_barang;
      page.drawText(name, { x: M + 28, y: y + 4, size: 8, font: font, color: gray700 });

      // Bar
      const barW = pct * barMaxW;
      page.drawRectangle({ x: M + 120, y: y, width: barW, height: barH, color: color });

      // Qty + revenue on right
      page.drawText(`${p.qty_sold}`, { x: M + 120 + barMaxW + 8, y: y + 4, size: 8, font: fontBold, color: gray700 });
      page.drawText(formatCurrency(p.revenue), { x: M + 120 + barMaxW + 40, y: y + 4, size: 8, font: font, color: gray500 });
      y -= barH + 4;
    });
    ...
    page.drawText('Total Produk:', { x: M + 6, y: y, size: 8, font: fontBold, color: gray700 });
    page.drawText(`${totalQtySold} unit`, { x: W - M - 60, y: y, size: 8, font: fontBold, color: primary });
  }
